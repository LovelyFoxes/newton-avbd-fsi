...
# PBF comparison set and intentionally reuses the same scene parameters as
# the IPBF version.
#
# Command: python -m newton.examples pbf_double_dam_break
#
###########################################################################
...
class Example:
    """Closed-box double dam break container for the PBF solver."""
    ...
    def __init__(self, viewer, args=None):
        ...
        base_substeps = int(particle_block["sim_substeps"])
        self.sim_substeps = base_substeps + (4 if bool(getattr(self.args, "test", False)) else 3)
        self.sim_dt = self.frame_dt / self.sim_substeps
        base_iterations = int(particle_block["iterations"])
        ...
        builder.default_shape_cfg.margin = float(particle_block["radius_mean"])
        builder.default_shape_cfg.gap = 2.0 * float(particle_block["radius_mean"])
        ...
        self.collision_pipeline = newton.examples.create_collision_pipeline(
            self.model,
            args,
            soft_contact_margin=max(0.06, 4.0 * float(particle_block["radius_mean"])),
        )
