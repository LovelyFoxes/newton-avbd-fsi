# SPDX-FileCopyrightText: Copyright (c) 2026 The Newton Developers
# SPDX-License-Identifier: Apache-2.0
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

###########################################################################
# Example FSI IPBF VBD Cloth Sphere Drop
#
# A high water pool fills the bottom of a tank. A thin retractable shelf
# initially separates the water from a high-resolution cloth sheet. A second
# thin shelf supports a rigid sphere above the cloth. The cloth shelf opens at
# frame 30 by default, then the sphere shelf opens at frame 90 so the sphere
# drops onto the floating cloth.
#
# Command: python -m newton.examples fsi_ipbf_vbd_cloth_sphere_drop
#
###########################################################################

from __future__ import annotations

import argparse

import numpy as np
import warp as wp

import newton
import newton.examples
from newton.examples.ipbf.common import (
    build_box_wireframe,
    get_particle_grid_half_span,
    get_particle_grid_origin_from_center,
    scale_velocities,
)
from newton.solvers import FSIBoundaryModel, SolverFSI, SolverIPBF, SolverVBD


def _quat_rotate_np(quat: wp.quat, vec: tuple[float, float, float]) -> np.ndarray:
    rot = np.array(wp.quat_to_matrix(quat), dtype=np.float32).reshape(3, 3)
    return rot @ np.array(vec, dtype=np.float32)


@wp.kernel
def apply_cloth_buoyancy_kernel(
    particle_q: wp.array(dtype=wp.vec3),
    particle_qd: wp.array(dtype=wp.vec3),
    particle_mass: wp.array(dtype=float),
    particle_f: wp.array(dtype=wp.vec3),
    particle_start: int,
    surface_y: float,
    support_depth: float,
    lift_scale: float,
    vertical_drag: float,
    gravity_magnitude: float,
):
    tid = wp.tid()
    particle = particle_start + tid
    mass = particle_mass[particle]
    if mass <= 0.0:
        return

    depth = surface_y - particle_q[particle][1]
    if depth <= 0.0:
        return

    immersion = wp.min(depth / support_depth, 1.0)
    lift = mass * gravity_magnitude * lift_scale * immersion
    drag = -mass * vertical_drag * particle_qd[particle][1]
    particle_f[particle] = particle_f[particle] + wp.vec3(0.0, lift + drag, 0.0)


class Example:
    """Layered cloth-fluid-rigid scene with staged shelf releases."""

    @staticmethod
    def create_parser():
        parser = newton.examples.create_parser()
        parser.add_argument(
            "--coupling-mode",
            choices=["loose", "interlinked"],
            default="interlinked",
            help="FSI scheduler mode used for the layered drop scene.",
        )
        parser.add_argument(
            "--coupling-iterations",
            type=int,
            default=3,
            help="Number of outer IPBF/AVBD feedback passes used per simulation substep.",
        )
        parser.add_argument(
            "--cloth-release-step",
            type=int,
            default=None,
            help="Frame step at which the cloth support shelf retracts.",
        )
        parser.add_argument(
            "--sphere-release-step",
            type=int,
            default=None,
            help="Frame step at which the sphere support shelf retracts.",
        )
        parser.add_argument(
            "--shelf-open-duration-frames",
            type=int,
            default=None,
            help="Number of frames used to retract each support shelf sideways.",
        )
        parser.add_argument(
            "--cloth-self-contact",
            action=argparse.BooleanOptionalAction,
            default=False,
            help="Enable cloth-owned VBD self-contact.",
        )
        parser.add_argument(
            "--show-boundary-samples",
            action=argparse.BooleanOptionalAction,
            default=False,
            help="Render FSI boundary samples for debugging.",
        )
        parser.add_argument(
            "--triangle-contact-relaxation",
            type=float,
            default=None,
            help="Override the IPBF cloth triangle-contact relaxation.",
        )
        parser.add_argument(
            "--cloth-support-mode",
            choices=["off", "light-buoyancy"],
            default="off",
            help="Example-level cloth support term. Use 'off' to inspect raw coupling only.",
        )
        parser.add_argument(
            "--enable-runtime-diagnostics",
            action=argparse.BooleanOptionalAction,
            default=None,
            help="Record finite-state diagnostics while the example runs.",
        )
        return parser

    def _get_scene_config(self) -> dict[str, object]:
        if bool(getattr(self.args, "test", False)):
            config = {
                "tank_half_width": 0.46,
                "tank_half_depth": 0.32,
                "tank_wall_height": 1.10,
                "tank_wall_thickness": 0.030,
                "pool_bottom_clearance": 0.030,
                "pool_dim_x": 22,
                "pool_dim_y": 9,
                "pool_dim_z": 14,
                "cell": 0.035,
                "mass": 0.042875,
                "radius_mean": 0.013,
                "smoothing_radius": 0.075,
                "shelf_half_thickness": 0.008,
                "shelf_seal_overlap": 0.010,
                "shelf_water_gap": 0.010,
                "cloth_dim_x": 18,
                "cloth_dim_z": 12,
                "cloth_cell": 0.040,
                "cloth_particle_mass": 0.00008,
                "cloth_particle_radius": 0.008,
                "cloth_sample_thickness": 0.055,
                "cloth_shelf_clearance": 0.001,
                "cloth_buoyancy_surface_offset": -0.004,
                "cloth_buoyancy_support_depth": 0.032,
                "cloth_buoyancy_lift_scale": 1.20,
                "cloth_buoyancy_vertical_drag": 1.60,
                "upper_shelf_height_gap": 0.075,
                "sphere_radius": 0.065,
                "sphere_density": 620.0,
                "sphere_shelf_clearance": 0.003,
                "cloth_release_step": 30,
                "sphere_release_step": 90,
                "shelf_open_duration_frames": 8,
                "rest_density": 1000.0,
                "ipbf_iterations": 6,
                "vbd_iterations": 6,
                "sim_substeps": 5,
                "velocity_damping": 0.998,
                "viscosity_coefficient": 0.0020,
                "xsph_coefficient": 0.004,
                "boundary_velocity_damping": 0.99,
                "fsi_pressure_reaction_relaxation": 1.5,
                "static_boundary_weight": 0.5,
                "triangle_contact_relaxation": 1.0,
                "boundary_spacing": 0.035,
                "water_render_radius_scale": 0.54,
                "boundary_render_radius_scale": 0.18,
                "shape_contact_ke": 3.0e4,
                "shape_contact_kd": 0.1,
                "shape_contact_gap": 0.002,
                "cloth_tri_ke": 160.0,
                "cloth_tri_ka": 160.0,
                "cloth_tri_kd": 0.24,
                "cloth_edge_ke": 14.0,
                "cloth_edge_kd": 0.06,
                "cloth_self_contact_radius": 0.018,
                "cloth_self_contact_margin": 0.030,
                "cloth_support_mode": "off",
                "expected_min_cloth_drop": 0.015,
                "expected_min_sphere_drop": 0.020,
                "expected_min_cloth_mean_y": 0.120,
            }
        else:
            config = {
                "tank_half_width": 0.78,
                "tank_half_depth": 0.50,
                "tank_wall_height": 1.60,
                "tank_wall_thickness": 0.035,
                "pool_bottom_clearance": 0.040,
                "pool_dim_x": 58,
                "pool_dim_y": 16,
                "pool_dim_z": 36,
                "cell": 0.024,
                "mass": 0.013824,
                "radius_mean": 0.009,
                "smoothing_radius": 0.048,
                "shelf_half_thickness": 0.008,
                "shelf_seal_overlap": 0.012,
                "shelf_water_gap": 0.012,
                "cloth_dim_x": 52,
                "cloth_dim_z": 34,
                "cloth_cell": 0.026,
                "cloth_particle_mass": 0.000050,
                "cloth_particle_radius": 0.006,
                "cloth_sample_thickness": 0.060,
                "cloth_shelf_clearance": 0.001,
                "cloth_buoyancy_surface_offset": -0.006,
                "cloth_buoyancy_support_depth": 0.040,
                "cloth_buoyancy_lift_scale": 1.30,
                "cloth_buoyancy_vertical_drag": 1.80,
                "upper_shelf_height_gap": 0.100,
                "sphere_radius": 0.095,
                "sphere_density": 620.0,
                "sphere_shelf_clearance": 0.004,
                "cloth_release_step": 30,
                "sphere_release_step": 90,
                "shelf_open_duration_frames": 10,
                "rest_density": 1000.0,
                "ipbf_iterations": 6,
                "vbd_iterations": 6,
                "sim_substeps": 5,
                "velocity_damping": 0.999,
                "viscosity_coefficient": 0.0025,
                "xsph_coefficient": 0.006,
                "boundary_velocity_damping": 0.97,
                "fsi_pressure_reaction_relaxation": 1.5,
                "static_boundary_weight": 0.20,
                "triangle_contact_relaxation": 1.0,
                "boundary_spacing": 0.024,
                "water_render_radius_scale": 0.54,
                "boundary_render_radius_scale": 0.18,
                "shape_contact_ke": 4.0e4,
                "shape_contact_kd": 0.1,
                "shape_contact_gap": 0.002,
                "cloth_tri_ke": 260.0,
                "cloth_tri_ka": 260.0,
                "cloth_tri_kd": 0.24,
                "cloth_edge_ke": 18.0,
                "cloth_edge_kd": 0.06,
                "cloth_self_contact_radius": 0.012,
                "cloth_self_contact_margin": 0.020,
                "cloth_support_mode": "off",
                "expected_min_cloth_drop": 0.020,
                "expected_min_sphere_drop": 0.040,
                "expected_min_cloth_mean_y": 0.180,
            }

        cloth_release_step = getattr(self.args, "cloth_release_step", None)
        if cloth_release_step is not None:
            config["cloth_release_step"] = max(0, int(cloth_release_step))

        sphere_release_step = getattr(self.args, "sphere_release_step", None)
        if sphere_release_step is not None:
            config["sphere_release_step"] = max(0, int(sphere_release_step))

        shelf_open_duration_frames = getattr(self.args, "shelf_open_duration_frames", None)
        if shelf_open_duration_frames is not None:
            config["shelf_open_duration_frames"] = max(1, int(shelf_open_duration_frames))

        triangle_contact_relaxation = getattr(self.args, "triangle_contact_relaxation", None)
        if triangle_contact_relaxation is not None:
            config["triangle_contact_relaxation"] = float(triangle_contact_relaxation)

        cloth_support_mode = getattr(self.args, "cloth_support_mode", None)
        if cloth_support_mode is not None:
            config["cloth_support_mode"] = str(cloth_support_mode)

        config["cloth_self_contact_enabled"] = bool(getattr(self.args, "cloth_self_contact", False))
        return config

    def __init__(self, viewer, args=None):
        self.fps = 60
        self.frame_dt = 1.0 / self.fps
        self.sim_time = 0.0
        self.frame_index = 0
        self.viewer = viewer
        self.viewer._paused = True
        self.args = args
        self._reset_key_prev = False
        self.config = self._get_scene_config()
        self.show_boundary_samples = bool(getattr(self.args, "show_boundary_samples", False))
        self.enable_runtime_diagnostics = bool(
            getattr(self.args, "enable_runtime_diagnostics", False) or bool(getattr(self.args, "test", False))
        )

        self.sim_substeps = int(self.config["sim_substeps"])
        self.sim_dt = self.frame_dt / self.sim_substeps
        self.floor_y = 0.0
        self.tank_half_width = float(self.config["tank_half_width"])
        self.tank_half_depth = float(self.config["tank_half_depth"])
        self.tank_wall_height = float(self.config["tank_wall_height"])
        self.tank_wall_thickness = float(self.config["tank_wall_thickness"])
        self.tank_top_y = self.floor_y + self.tank_wall_height
        shelf_seal_overlap = float(self.config["shelf_seal_overlap"])
        self.shelf_half_extents = (
            self.tank_half_width + shelf_seal_overlap,
            float(self.config["shelf_half_thickness"]),
            self.tank_half_depth + shelf_seal_overlap,
        )

        self._compute_layer_heights()
        self._build_model()
        self._build_solvers()
        self._build_visualization_data()

        self.viewer.set_model(self.model)
        self.viewer.show_particles = True
        self.viewer.show_triangles = True
        self.viewer.set_camera(pos=wp.vec3(1.25, 1.25, 1.75), pitch=-25.0, yaw=-138.0)
        self.reset()

    def _compute_layer_heights(self) -> None:
        _, pool_half_span_y, _ = get_particle_grid_half_span(
            dim_x=int(self.config["pool_dim_x"]),
            dim_y=int(self.config["pool_dim_y"]),
            dim_z=int(self.config["pool_dim_z"]),
            cell_x=float(self.config["cell"]),
            cell_y=float(self.config["cell"]),
            cell_z=float(self.config["cell"]),
        )
        self.pool_center_y = self.floor_y + float(self.config["pool_bottom_clearance"]) + pool_half_span_y
        self.pool_surface_y = self.pool_center_y + pool_half_span_y + float(self.config["radius_mean"])
        self.cloth_shelf_y = (
            self.pool_surface_y + float(self.config["shelf_water_gap"]) + float(self.config["shelf_half_thickness"])
        )
        self.cloth_center_y = (
            self.cloth_shelf_y
            + float(self.config["shelf_half_thickness"])
            + float(self.config["cloth_particle_radius"])
            + float(self.config["cloth_shelf_clearance"])
        )
        self.sphere_shelf_y = self.cloth_shelf_y + float(self.config["upper_shelf_height_gap"])
        self.sphere_center_y = (
            self.sphere_shelf_y
            + float(self.config["shelf_half_thickness"])
            + float(self.config["sphere_radius"])
            + float(self.config["sphere_shelf_clearance"])
        )

    def _contact_shape_cfg(self, *, density: float, mu: float = 0.0) -> newton.ModelBuilder.ShapeConfig:
        return newton.ModelBuilder.ShapeConfig(
            density=density,
            ke=float(self.config["shape_contact_ke"]),
            kd=float(self.config["shape_contact_kd"]),
            mu=mu,
            gap=float(self.config["shape_contact_gap"]),
        )

    def _add_tank(self, builder: newton.ModelBuilder) -> None:
        wall_cfg = self._contact_shape_cfg(density=0.0)
        hx = self.tank_half_width
        hz = self.tank_half_depth
        hy = 0.5 * self.tank_wall_height
        wt = self.tank_wall_thickness
        center_y = self.floor_y + hy

        builder.add_shape_box(
            body=-1,
            xform=wp.transform((0.0, self.floor_y - wt, 0.0), wp.quat_identity()),
            hx=hx + wt,
            hy=wt,
            hz=hz + wt,
            cfg=wall_cfg,
        )
        builder.add_shape_box(
            body=-1,
            xform=wp.transform((hx + wt, center_y, 0.0), wp.quat_identity()),
            hx=wt,
            hy=hy + wt,
            hz=hz + wt,
            cfg=wall_cfg,
        )
        builder.add_shape_box(
            body=-1,
            xform=wp.transform((-(hx + wt), center_y, 0.0), wp.quat_identity()),
            hx=wt,
            hy=hy + wt,
            hz=hz + wt,
            cfg=wall_cfg,
        )
        builder.add_shape_box(
            body=-1,
            xform=wp.transform((0.0, center_y, hz + wt), wp.quat_identity()),
            hx=hx,
            hy=hy + wt,
            hz=wt,
            cfg=wall_cfg,
        )
        builder.add_shape_box(
            body=-1,
            xform=wp.transform((0.0, center_y, -(hz + wt)), wp.quat_identity()),
            hx=hx,
            hy=hy + wt,
            hz=wt,
            cfg=wall_cfg,
        )

    def _add_support_shelf(
        self,
        builder: newton.ModelBuilder,
        *,
        label: str,
        center_y: float,
    ) -> int:
        shelf_body = builder.add_body(
            xform=wp.transform((0.0, center_y, 0.0), wp.quat_identity()),
            is_kinematic=True,
            label=label,
        )
        builder.add_shape_box(
            body=shelf_body,
            hx=self.shelf_half_extents[0],
            hy=self.shelf_half_extents[1],
            hz=self.shelf_half_extents[2],
            cfg=self._contact_shape_cfg(density=0.0),
        )
        return shelf_body

    def _add_pool(self, builder: newton.ModelBuilder) -> None:
        self.fluid_particle_start = builder.particle_count
        builder.add_particle_grid(
            pos=get_particle_grid_origin_from_center(
                center=(0.0, self.pool_center_y, 0.0),
                dim_x=int(self.config["pool_dim_x"]),
                dim_y=int(self.config["pool_dim_y"]),
                dim_z=int(self.config["pool_dim_z"]),
                cell_x=float(self.config["cell"]),
                cell_y=float(self.config["cell"]),
                cell_z=float(self.config["cell"]),
            ),
            rot=wp.quat_identity(),
            vel=wp.vec3(0.0, 0.0, 0.0),
            dim_x=int(self.config["pool_dim_x"]),
            dim_y=int(self.config["pool_dim_y"]),
            dim_z=int(self.config["pool_dim_z"]),
            cell_x=float(self.config["cell"]),
            cell_y=float(self.config["cell"]),
            cell_z=float(self.config["cell"]),
            mass=float(self.config["mass"]),
            jitter=0.0,
            radius_mean=float(self.config["radius_mean"]),
            flags=int(newton.ParticleFlags.ACTIVE),
        )
        self.fluid_particle_count = builder.particle_count - self.fluid_particle_start

    def _add_cloth(self, builder: newton.ModelBuilder) -> None:
        self.cloth_particle_start = builder.particle_count
        self.cloth_triangle_start = builder.tri_count
        dim_x = int(self.config["cloth_dim_x"])
        dim_z = int(self.config["cloth_dim_z"])
        cell = float(self.config["cloth_cell"])
        cloth_width = dim_x * cell
        cloth_depth = dim_z * cell
        cloth_rot = wp.quat_from_axis_angle(wp.vec3(1.0, 0.0, 0.0), 0.5 * np.pi)
        cloth_center = np.array((0.0, self.cloth_center_y, 0.0), dtype=np.float32)
        cloth_pos = cloth_center - _quat_rotate_np(cloth_rot, (0.5 * cloth_width, 0.5 * cloth_depth, 0.0))

        builder.add_cloth_grid(
            pos=wp.vec3(*cloth_pos),
            rot=cloth_rot,
            vel=wp.vec3(0.0, 0.0, 0.0),
            dim_x=dim_x,
            dim_y=dim_z,
            cell_x=cell,
            cell_y=cell,
            mass=float(self.config["cloth_particle_mass"]),
            fix_left=False,
            fix_right=False,
            fix_top=False,
            fix_bottom=False,
            tri_ke=float(self.config["cloth_tri_ke"]),
            tri_ka=float(self.config["cloth_tri_ka"]),
            tri_kd=float(self.config["cloth_tri_kd"]),
            edge_ke=float(self.config["cloth_edge_ke"]),
            edge_kd=float(self.config["cloth_edge_kd"]),
            particle_radius=float(self.config["cloth_particle_radius"]),
        )
        self.cloth_particle_count = builder.particle_count - self.cloth_particle_start
        self.cloth_triangle_count = builder.tri_count - self.cloth_triangle_start

    def _add_sphere(self, builder: newton.ModelBuilder) -> None:
        self.sphere_body = builder.add_body(
            xform=wp.transform((0.0, self.sphere_center_y, 0.0), wp.quat_identity()),
            label="fsi_layered_drop_sphere",
        )
        builder.add_shape_sphere(
            body=self.sphere_body,
            radius=float(self.config["sphere_radius"]),
            cfg=self._contact_shape_cfg(density=float(self.config["sphere_density"])),
        )

    def _build_model(self) -> None:
        builder = newton.ModelBuilder(up_axis=newton.Axis.Y)
        SolverIPBF.register_custom_attributes(builder)
        builder.rigid_gap = float(self.config["shape_contact_gap"])
        builder.default_shape_cfg.mu = 0.0
        builder.default_shape_cfg.ke = float(self.config["shape_contact_ke"])
        builder.default_shape_cfg.kd = float(self.config["shape_contact_kd"])
        builder.default_shape_cfg.gap = float(self.config["shape_contact_gap"])

        self._add_tank(builder)
        self.cloth_shelf_body = self._add_support_shelf(
            builder,
            label="fsi_layered_drop_cloth_shelf",
            center_y=self.cloth_shelf_y,
        )
        self.sphere_shelf_body = self._add_support_shelf(
            builder,
            label="fsi_layered_drop_sphere_shelf",
            center_y=self.sphere_shelf_y,
        )
        self._add_pool(builder)
        self._add_cloth(builder)
        self._add_sphere(builder)
        builder.color(include_bending=True)

        self.model = builder.finalize()
        self.model.set_gravity((0.0, -9.81, 0.0))

    def _build_cloth_self_contact_vertex_filtering_map(self) -> dict[int, list[int]] | None:
        if self.cloth_triangle_count <= 0 or self.fluid_particle_count <= 0:
            return None

        cloth_triangle_ids = list(range(self.cloth_triangle_start, self.cloth_triangle_start + self.cloth_triangle_count))
        return dict.fromkeys(
            range(self.fluid_particle_start, self.fluid_particle_start + self.fluid_particle_count),
            cloth_triangle_ids,
        )

    def _build_solvers(self) -> None:
        cloth_self_contact_vertex_filtering_map = None
        if bool(self.config["cloth_self_contact_enabled"]):
            cloth_self_contact_vertex_filtering_map = self._build_cloth_self_contact_vertex_filtering_map()

        self.boundary_model = FSIBoundaryModel(
            self.model,
            spacing=float(self.config["boundary_spacing"]),
            support_radius=float(self.config["smoothing_radius"]),
            hydrostatic_volume_mode=FSIBoundaryModel.HydrostaticVolumeMode.DYNAMIC_SHAPE_SURFACE_QUADRATURE,
            include_static=True,
            include_dynamic=True,
            include_triangles=True,
            deformable_sample_thickness=float(self.config["cloth_sample_thickness"]),
            device=self.model.device,
        )
        self.fluid_solver = SolverIPBF(
            self.model,
            SolverIPBF.Config(
                rest_density=float(self.config["rest_density"]),
                smoothing_radius=float(self.config["smoothing_radius"]),
                compliance=1.0e-5,
                iterations=int(self.config["ipbf_iterations"]),
                relaxation=0.5,
                viscosity_coefficient=float(self.config["viscosity_coefficient"]),
                xsph_coefficient=float(self.config["xsph_coefficient"]),
                boundary_velocity_damping=float(self.config["boundary_velocity_damping"]),
                fsi_pressure_reaction_relaxation=float(self.config["fsi_pressure_reaction_relaxation"]),
                fsi_static_boundary_weight=float(self.config["static_boundary_weight"]),
                fsi_triangle_contact_enabled=True,
                fsi_triangle_contact_margin=0.0,
                fsi_triangle_contact_relaxation=float(self.config["triangle_contact_relaxation"]),
                fluid_particle_start=self.fluid_particle_start,
                fluid_particle_count=self.fluid_particle_count,
            ),
            boundary_model=self.boundary_model,
        )
        self.solid_solver = SolverVBD(
            self.model,
            iterations=int(self.config["vbd_iterations"]),
            particle_start=self.cloth_particle_start,
            particle_count=self.cloth_particle_count,
            particle_enable_self_contact=bool(self.config["cloth_self_contact_enabled"]),
            particle_self_contact_radius=float(self.config["cloth_self_contact_radius"]),
            particle_self_contact_margin=float(self.config["cloth_self_contact_margin"]),
            particle_external_vertex_contact_filtering_map=cloth_self_contact_vertex_filtering_map,
            fsi_boundary_model=self.boundary_model,
        )
        coupling_mode = getattr(self.args, "coupling_mode", "interlinked")
        self.solver = SolverFSI(
            self.model,
            fluid_solver=self.fluid_solver,
            solid_solver=self.solid_solver,
            boundary_model=self.boundary_model,
            config=SolverFSI.Config(
                mode=(
                    SolverFSI.Config.CouplingMode.INTERLINKED
                    if coupling_mode == "interlinked"
                    else SolverFSI.Config.CouplingMode.LOOSE
                ),
                coupling_iterations=max(1, int(getattr(self.args, "coupling_iterations", 3))),
            ),
        )
        self.collision_pipeline = newton.examples.create_collision_pipeline(
            self.model,
            self.args,
            soft_contact_margin=max(float(self.config["radius_mean"]), float(self.config["cloth_particle_radius"])) * 2.0,
            rigid_contact_max=8192,
        )
        self.state_0 = self.model.state()
        self.state_1 = self.model.state()
        self.contacts = self.model.contacts(collision_pipeline=self.collision_pipeline)

    def _build_visualization_data(self) -> None:
        self.tank_wire_starts, self.tank_wire_ends = build_box_wireframe(
            min_x=-self.tank_half_width,
            max_x=self.tank_half_width,
            min_y=self.floor_y,
            max_y=self.tank_top_y,
            min_z=-self.tank_half_depth,
            max_z=self.tank_half_depth,
            include_top=True,
            device=self.model.device,
        )
        tri_indices = self.model.tri_indices.numpy()[self.cloth_triangle_start : self.cloth_triangle_start + self.cloth_triangle_count]
        self.cloth_mesh_indices = wp.array(tri_indices.reshape(-1), dtype=wp.int32, device=self.model.device)
        self.fluid_colors = wp.full(
            self.fluid_particle_count,
            value=wp.vec3(0.20, 0.62, 1.0),
            dtype=wp.vec3,
            device=self.model.device,
        )
        self.fluid_radii = wp.full(
            self.fluid_particle_count,
            value=float(self.config["radius_mean"]) * float(self.config["water_render_radius_scale"]),
            dtype=wp.float32,
            device=self.model.device,
        )
        self.shelf_colors = wp.array(
            [wp.vec3(0.80, 0.82, 0.86), wp.vec3(0.72, 0.75, 0.80)],
            dtype=wp.vec3,
            device=self.model.device,
        )
        self.sphere_colors = wp.array([wp.vec3(0.95, 0.50, 0.18)], dtype=wp.vec3, device=self.model.device)
        self.shape_material = wp.array([wp.vec4(0.42, 0.0, 0.0, 0.0)], dtype=wp.vec4, device=self.model.device)
        self.boundary_radii = wp.full(
            self.boundary_model.sample_count,
            value=float(self.config["boundary_spacing"]) * float(self.config["boundary_render_radius_scale"]),
            dtype=wp.float32,
            device=self.model.device,
        )
        sample_body = self.boundary_model.sample_body.numpy()
        boundary_colors = np.zeros((self.boundary_model.sample_count, 3), dtype=np.float32)
        boundary_colors[sample_body < 0] = np.array([0.92, 0.94, 0.98], dtype=np.float32)
        boundary_colors[sample_body >= 0] = np.array([1.0, 0.48, 0.18], dtype=np.float32)
        self.boundary_colors = wp.array(boundary_colors, dtype=wp.vec3, device=self.model.device)

    def _body_xforms(self, body_indices: list[int]) -> wp.array(dtype=wp.transform):
        body_q = self.state_0.body_q.numpy()[body_indices]
        return wp.array(body_q, dtype=wp.transform, device=self.model.device)

    def _set_body_pose(
        self,
        state: newton.State,
        body_idx: int,
        center: tuple[float, float, float],
        linear_velocity: tuple[float, float, float],
    ) -> None:
        body_q_np = state.body_q.numpy()
        body_q_np[body_idx, 0] = float(center[0])
        body_q_np[body_idx, 1] = float(center[1])
        body_q_np[body_idx, 2] = float(center[2])
        body_q_np[body_idx, 3:7] = np.array(wp.quat_identity(), dtype=np.float32)
        state.body_q = wp.array(body_q_np, dtype=wp.transform, device=self.model.device)

        body_qd_np = state.body_qd.numpy()
        body_qd_np[body_idx, 0:3] = np.array(linear_velocity, dtype=np.float32)
        body_qd_np[body_idx, 3:6] = 0.0
        state.body_qd = wp.array(body_qd_np, dtype=wp.spatial_vector, device=self.model.device)

    def _release_alpha(self, release_step: int, duration_frames: int) -> float:
        if self.frame_index < release_step:
            return 0.0
        if duration_frames <= 1:
            return 1.0
        return min(1.0, float(self.frame_index - release_step + 1) / float(duration_frames))

    def _shelf_center_and_velocity(
        self,
        *,
        closed_y: float,
        release_step: int,
    ) -> tuple[tuple[float, float, float], tuple[float, float, float]]:
        duration = int(self.config["shelf_open_duration_frames"])
        del duration
        if self.frame_index < release_step:
            return (0.0, closed_y, 0.0), (0.0, 0.0, 0.0)

        # Treat the shelf as an instantly removed support: move it far outside
        # the tank with zero velocity so it does not sweep through the scene.
        hidden_x = 4.0 * self.tank_half_width + 2.0 * self.shelf_half_extents[0]
        return (hidden_x, closed_y, 0.0), (0.0, 0.0, 0.0)

    def _apply_shelf_states(self, state: newton.State) -> None:
        cloth_center, cloth_velocity = self._shelf_center_and_velocity(
            closed_y=self.cloth_shelf_y,
            release_step=int(self.config["cloth_release_step"]),
        )
        sphere_center, sphere_velocity = self._shelf_center_and_velocity(
            closed_y=self.sphere_shelf_y,
            release_step=int(self.config["sphere_release_step"]),
        )
        self._set_body_pose(state, self.cloth_shelf_body, cloth_center, cloth_velocity)
        self._set_body_pose(state, self.sphere_shelf_body, sphere_center, sphere_velocity)

    def _apply_cloth_buoyancy(self, state: newton.State) -> None:
        if str(self.config["cloth_support_mode"]) == "off":
            return
        if self.frame_index < int(self.config["cloth_release_step"]):
            return

        wp.launch(
            apply_cloth_buoyancy_kernel,
            dim=self.cloth_particle_count,
            inputs=[
                state.particle_q,
                state.particle_qd,
                self.model.particle_mass,
                state.particle_f,
                int(self.cloth_particle_start),
                float(self.pool_surface_y + float(self.config["cloth_buoyancy_surface_offset"])),
                float(self.config["cloth_buoyancy_support_depth"]),
                float(self.config["cloth_buoyancy_lift_scale"]),
                float(self.config["cloth_buoyancy_vertical_drag"]),
                9.81,
            ],
            device=self.model.device,
        )

    def gui(self, ui):
        if ui.button("Reset"):
            self.reset()
        ui.text("Layered cloth-fluid-rigid drop")
        ui.text(f"Cloth release: {int(self.config['cloth_release_step'])}")
        ui.text(f"Sphere release: {int(self.config['sphere_release_step'])}")
        ui.text(f"Shelf full thickness: {2.0 * float(self.config['shelf_half_thickness']):.3f} m")
        ui.text(f"Cloth support: {self.config['cloth_support_mode']}")
        ui.text(f"Fluid particles: {self.fluid_particle_count}")
        ui.text(f"Cloth particles: {self.cloth_particle_count}")
        ui.text(f"Show boundary samples: {'on' if self.show_boundary_samples else 'off'}")

    def reset(self):
        self.sim_time = 0.0
        self.frame_index = 0
        self.fluid_solver.reset(self.state_0)
        self.fluid_solver.reset(self.state_1)
        self.solid_solver.reset(self.state_0)
        self.solid_solver.reset(self.state_1)
        self._apply_shelf_states(self.state_0)
        self._apply_shelf_states(self.state_1)
        self.contacts.clear()
        self.boundary_model.clear_forces()
        self.boundary_model.update_world_kinematics(self.state_0)
        self.boundary_model.build_grid()

        particle_q = self.state_0.particle_q.numpy()
        body_q = self.state_0.body_q.numpy()
        self.initial_cloth_mean_y = float(np.mean(particle_q[self.cloth_particle_start :, 1]))
        self.initial_sphere_y = float(body_q[self.sphere_body, 1])
        self.min_cloth_mean_y = self.initial_cloth_mean_y
        self.min_sphere_y = self.initial_sphere_y
        self.max_triangle_contact_pair_count = 0
        self.max_fsi_particle_inertia_offset = 0.0
        self.max_vertex_force_norm = 0.0
        self.max_density = 0.0
        self.max_particle_speed = 0.0
        self.max_body_linear_speed = 0.0
        self.states_remain_finite = True
        self.viewer._paused = True

    def capture(self):
        self.graph = None

    def _record_substep_diagnostics(self) -> None:
        if not self.enable_runtime_diagnostics:
            return
        triangle_pair_count = int(self.fluid_solver._triangle_contact_pair_count.numpy()[0])
        vertex_force = self.boundary_model.vertex_force.numpy()[self.cloth_particle_start :]
        fsi_inertia_offset = self.solid_solver.fsi_particle_inertia_offset.numpy()[self.cloth_particle_start :]
        density = self.state_0.ipbf.density.numpy()
        particle_q = self.state_0.particle_q.numpy()
        particle_qd = self.state_0.particle_qd.numpy()
        body_q = self.state_0.body_q.numpy()
        body_qd = self.state_0.body_qd.numpy()

        self.max_triangle_contact_pair_count = max(self.max_triangle_contact_pair_count, triangle_pair_count)
        self.max_fsi_particle_inertia_offset = max(
            self.max_fsi_particle_inertia_offset,
            float(np.linalg.norm(fsi_inertia_offset, axis=1).max()),
        )
        self.max_vertex_force_norm = max(self.max_vertex_force_norm, float(np.linalg.norm(vertex_force, axis=1).max()))
        self.max_density = max(self.max_density, float(np.max(density)))
        self.max_particle_speed = max(self.max_particle_speed, float(np.linalg.norm(particle_qd, axis=1).max()))
        self.max_body_linear_speed = max(self.max_body_linear_speed, float(np.linalg.norm(body_qd[:, :3], axis=1).max()))
        self.states_remain_finite = self.states_remain_finite and bool(
            np.isfinite(vertex_force).all()
            and np.isfinite(fsi_inertia_offset).all()
            and np.isfinite(density).all()
            and np.isfinite(particle_q).all()
            and np.isfinite(particle_qd).all()
            and np.isfinite(body_q).all()
            and np.isfinite(body_qd).all()
        )

    def _record_frame_diagnostics(self) -> None:
        particle_q = self.state_0.particle_q.numpy()
        body_q = self.state_0.body_q.numpy()
        cloth_q = particle_q[self.cloth_particle_start : self.cloth_particle_start + self.cloth_particle_count]
        self.min_cloth_mean_y = min(self.min_cloth_mean_y, float(np.mean(cloth_q[:, 1])))
        self.min_sphere_y = min(self.min_sphere_y, float(body_q[self.sphere_body, 1]))

    def simulate(self):
        for _ in range(self.sim_substeps):
            self._apply_shelf_states(self.state_0)
            self.state_0.clear_forces()
            self._apply_cloth_buoyancy(self.state_0)
            self.viewer.apply_forces(self.state_0)
            self.model.collide(self.state_0, self.contacts)
            wp.launch(
                scale_velocities,
                dim=self.model.particle_count,
                inputs=[self.state_0.particle_qd, float(self.config["velocity_damping"])],
                device=self.model.device,
            )
            self.solver.step(self.state_0, self.state_1, control=None, contacts=self.contacts, dt=self.sim_dt)
            self._record_substep_diagnostics()
            self.state_0, self.state_1 = self.state_1, self.state_0

    def step(self):
        if hasattr(self.viewer, "is_key_down"):
            reset_down = bool(self.viewer.is_key_down("r"))
            if reset_down and not self._reset_key_prev:
                self.reset()
            self._reset_key_prev = reset_down

        self.simulate()
        self._record_frame_diagnostics()
        self.frame_index += 1
        self.sim_time += self.frame_dt

    def test_final(self):
        assert self.states_remain_finite, "layered drop scene produced non-finite diagnostics"
        if self.enable_runtime_diagnostics:
            assert self.max_density > 0.0, "IPBF density diagnostics were not updated"

        cloth_release_done = self.frame_index > int(self.config["cloth_release_step"]) + int(
            self.config["shelf_open_duration_frames"]
        )
        if cloth_release_done:
            cloth_drop = self.initial_cloth_mean_y - self.min_cloth_mean_y
            assert cloth_drop > float(self.config["expected_min_cloth_drop"]), f"cloth did not drop enough: {cloth_drop}"
            if str(self.config["cloth_support_mode"]) == "off":
                assert self.max_triangle_contact_pair_count > 0, "cloth-fluid triangle contacts were never collected"
                assert self.max_fsi_particle_inertia_offset > 0.0, "cloth triangle-contact deltas never reached VBD"
                assert self.max_vertex_force_norm > 0.0, "cloth pressure/contact reaction never reached VBD"
            else:
                assert self.min_cloth_mean_y > float(self.config["expected_min_cloth_mean_y"]), (
                    f"cloth sank too deep: min_mean_y={self.min_cloth_mean_y}"
                )

        sphere_release_done = self.frame_index > int(self.config["sphere_release_step"]) + int(
            self.config["shelf_open_duration_frames"]
        )
        if sphere_release_done:
            sphere_drop = self.initial_sphere_y - self.min_sphere_y
            assert sphere_drop > float(self.config["expected_min_sphere_drop"]), (
                f"sphere did not drop enough: {sphere_drop}"
            )

    def render(self):
        self.viewer.begin_frame(self.sim_time)
        self.viewer.log_lines(
            "/fsi/layered_drop_tank_wire",
            self.tank_wire_starts,
            self.tank_wire_ends,
            colors=(0.86, 0.90, 0.95),
            width=0.010,
        )
        self.viewer.log_mesh(
            "/fsi/layered_drop_cloth_mesh",
            points=self.state_0.particle_q,
            indices=self.cloth_mesh_indices,
            hidden=not self.viewer.show_triangles,
            backface_culling=False,
        )
        self.viewer.log_shapes(
            "/fsi/layered_drop_shelves",
            newton.GeoType.BOX,
            self.shelf_half_extents,
            self._body_xforms([self.cloth_shelf_body, self.sphere_shelf_body]),
            self.shelf_colors,
            self.shape_material,
        )
        self.viewer.log_shapes(
            "/fsi/layered_drop_sphere",
            newton.GeoType.SPHERE,
            float(self.config["sphere_radius"]),
            self._body_xforms([self.sphere_body]),
            self.sphere_colors,
            self.shape_material,
        )
        self.viewer.log_points(
            "/fsi/layered_drop_water_points",
            points=wp.array(
                self.state_0.particle_q.numpy()[self.fluid_particle_start : self.fluid_particle_start + self.fluid_particle_count],
                dtype=wp.vec3,
                device=self.model.device,
            ),
            radii=self.fluid_radii,
            colors=self.fluid_colors,
            hidden=not self.viewer.show_particles,
        )
        self.viewer.log_points(
            "/fsi/layered_drop_boundary_samples",
            points=self.boundary_model.sample_x_world,
            radii=self.boundary_radii,
            colors=self.boundary_colors,
            hidden=not (self.viewer.show_particles and self.show_boundary_samples),
        )
        self.viewer.end_frame()


if __name__ == "__main__":
    parser = Example.create_parser()
    viewer, args = newton.examples.init(parser)
    example = Example(viewer, args)
    newton.examples.run(example, args)
